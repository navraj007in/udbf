...
Public Class SQLite
    ...
    End Sub
    Public Sub New(ByVal pConnString As String, ByVal pDebugMode As DebugModes)
        Me.ConnString = pConnString
    ...
    Public Overrides Function InsertS(ByVal Table As String, ByVal Fields() As String, ByVal Values() As String, ByVal Count As Integer) As Integer
        ...
        Try
            ...
            com.ExecuteNonQuery()
        Catch ex As Exception
            If (DebugMode = DebugModes.Desktop) Then
                MsgBox("Error :" & ex.StackTrace & " ." & vbCrLf & "Error Details :" & ex.Message)
            Else
                Log(vbCrLf & "Error :" & ex.StackTrace & vbCrLf & "Details :" & ex.Message & vbCrLf & "Query String :" & sql)
            End If
            conn.Dispose()
            conn.Close()
            Throw New UDBFException(ex.Message)
        Finally
            conn.Dispose()
            conn.Close()
        End Try
    ...
    Public Overrides Function SelectDate(ByVal Query As String) As Date
        Dim Result As New DataTable
        Dim conn As New sqliteconnection(Me.ConnString)
        Dim Reader As SQLiteDataAdapter
        Dim com As New SQLiteCommand
        Try
            conn.Open()
            com.Connection = conn
            com.CommandText = Query
            Reader = New SQLiteDataAdapter(com)
            Reader.Fill(Result)
    ...
    Public Function SelectDataSet(ByVal Query As String) As System.Data.DataTable
        Dim Result As New DataTable
        Dim conn As New SQLiteConnection
        Dim Reader As New SQLiteDataAdapter
        Dim com As New SQLiteCommand
    ...
    Public Overloads Overrides Sub writeXML(ByVal Path As String, ByVal Query As String)
        ...
        ' Dim myAdapter1 As New SQLiteDataAdapter
        ' Dim dt As String
    ...
    Public Overloads Overrides Function ConvertDate(ByVal sDate As Date) As String
        Dim sMonth As String = sDate.Month
        Dim sDay As String = sDate.Day
        If sDate.Month.ToString.Length = 1 Then sMonth = "0" & sDate.Month
        If sDate.Day.ToString.Length = 1 Then sDay = "0" & sDate.Day
        Dim ssDate As String = sDate.Year & "-" & sMonth & "-" & sDay & " 12:00:00"
        Return ssDate
    End Function
    Public Overloads Function LoadComboBox(ByVal Table As String, ByVal strField As String, ByVal intField As String, ByVal Combo As System.Web.UI.WebControls.DropDownList) As Integer
        Dim conn As New SQLiteConnection
        Dim Reader As SQLiteDataReader
        Dim com As New SQLiteCommand
        Dim Query As String
        ...
        Try
            ...
            Reader = com.ExecuteReader
            While Reader.Read
                Dim li As New System.Web.UI.WebControls.ListItem(Reader.GetValue(0), Reader.GetValue(1))
                Combo.Items.Add(li)
        ...
        Return 0
    End Function
    Public Overloads Function LoadComboBox(ByVal Table As String, ByVal strField As String, ByVal intField As String, ByVal Combo As System.Web.UI.WebControls.DropDownList, ByVal Where As String) As Integer
        Dim conn As New SQLiteConnection
        Dim Reader As SQLiteDataReader
        Dim com As New SQLiteCommand
        Dim Query As String
    ...
    Public Overloads Function LoadComboBox(ByVal Table As String, ByVal Field As String, ByVal Combo As System.Web.UI.WebControls.DropDownList) As Integer
        Dim conn As New SQLiteConnection
        Dim Reader As SQLiteDataReader
        Dim com As New SQLiteCommand
        conn.ConnectionString = Me.ConnString
        ...
        Return 0
    End Function
    Public Overloads Function LoadDropDown(ByVal Table As String, ByVal Field As String, ByVal Combo As System.Web.UI.WebControls.DropDownList, ByVal Where As String) As Integer
        Dim conn As New SQLiteConnection
        Dim Reader As SQLiteDataReader
        Dim com As New SQLiteCommand
        conn.ConnectionString = Me.ConnString
